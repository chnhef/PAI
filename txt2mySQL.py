...
t_lines = len(t_data)
print('\r' + "请稍候，正在转换个人客户信息，源文件共 %s 条记录……" % (t_lines))
...
t_lines = len(t_data)
print('\r'"请稍候，正在转换贷款协议信息，源文件共 %s 条记录……" % (t_lines))
for i in tqdm(range(2, t_lines + 2)):
    ...
    value = (
        t_list[0], t_list[1], t_list[2], t_list[3], t_list[4], t_list[5], t_list[6], t_list[7], t_list[8], t_list[9], t_list[10], t_list[11],
        t_list[12], t_list[13], t_list[14], t_list[15], t_list[16], t_list[17])
    cur.execute(sqli, value)
    con.commit()
...
t_lines = len(t_data)
print('\r' + "请稍候，正在转换贷款借据信息，源文件共 %s 条记录……" % (t_lines))
for i in tqdm(range(2, t_lines + 2)):
    t_list = linecache.getline(file_iou, i).strip('\n').split(',')
    sqli = 'insert into PAI.loan_iou(loan_id,iou_id,iou_amount,iou_currency,iou_remamount,iou_rate,iou_term,iou_termunit,iou_method,iou_repayorder,iou_defrate,iou_prepayflag,iou_prepayfee,iou_changeflag,iou_changefee,iou_effdate,iou_duedate,iou_status,iou_defstart,iou_defcount,iou_defint,iou_defprin,iou_defamount) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
    value = (
        t_list[0], t_list[1], t_list[2], t_list[3], t_list[4], t_list[5], t_list[6], t_list[7], t_list[8], t_list[9], t_list[10], t_list[11],
        t_list[12], t_list[13], t_list[14], t_list[15], t_list[16], t_list[17], t_list[18], t_list[19], t_list[20], t_list[21], t_list[22])
    cur.execute(sqli, value)
    con.commit()
...
t_lines = len(t_data)
print('\r' + "请稍候，正在转换还款计划信息，源文件共 %s 条记录……" % (t_lines))
for i in tqdm(range(2, t_lines + 2)):
...
t_lines = len(t_data)
print('\r' + "请稍候，正在转换还款历史信息，源文件共 %s 条记录……" % (t_lines))
for i in tqdm(range(2, t_lines + 2)):
    t_list = linecache.getline(file_repay, i).strip('\n').split(',')
    sqli = 'insert into PAI.repay_history(loan_id,iou_id,return_bill,return_date,return_int,return_prin,return_amount,return_flag) values (%s,%s,%s,%s,%s,%s,%s,%s)'
    value = (t_list[0], t_list[1], t_list[2], t_list[3], t_list[4], t_list[5], t_list[6], t_list[7])
    cur.execute(sqli, value)
    con.commit()
...
t_lines = len(t_data)
print('\r' + "请稍候，正在转换违约历史信息，源文件共 %s 条记录……" % (t_lines))
for i in tqdm(range(2, t_lines + 2)):
...
print("\r" + "违约历史信息已转换完成！共输出 %s 条记录。" % (cur.fetchone()))
